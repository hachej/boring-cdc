...
def main():
 ...
 binary=ROOT/'target/debug/examples/m2_spool_component';modes=['near-limit','filesystem-isolation','oversized','policy-vectors'] if mode=='e2e' else ['enospc','startup'];commands=[];observed={};displays=[]
 ...
 if mode=='e2e':
  ...
  assert observed['filesystem-isolation']['same_filesystem_blocked'] and observed['filesystem-isolation']['first_filesystem_reserved']==100 and observed['filesystem-isolation']['second_filesystem_reserved']==800
  assert observed['oversized']['outcome']['feedback_permitted'] is False and observed['oversized']['prepared_persistence']
  assert observed['policy-vectors']['same_fingerprint']=='Suppressed' and not observed['policy-vectors']['same_fingerprint_write']
  assert observed['policy-vectors']['unchanged_limit']=='require_reseed' and observed['policy-vectors']['wal_missing']=='require_reseed'
  assert observed['policy-vectors']['changed_limit']=='retained_wal_rearmed' and observed['policy-vectors']['changed_write']
 else:
