...
fp=sha(canon(cfg));before={'generation_state':'copying','pending_chunks':3,'active_workers':0};after={k:observed[k] for k in ('generation_state','bounded_reader_released','atomic_chunk_event_commit')};after.update({k:observed[k] for k in ('stale_completion_rejected','snapshot_events','complete_chunks','remaining_claims') if k in observed});after['keyset_only']=observed.get('postgres_keyset_runs')==2
