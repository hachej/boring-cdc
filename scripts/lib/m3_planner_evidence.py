...
if mode=='e2e':
 assert observed['generation_state']=='fencing' and observed['snapshot_events']==5 and observed['complete_chunks']==3 and observed['remaining_claims']==0 and observed['rust_worker_runs']==2 and observed['postgres_keyset_runs']==2 and observed['exported_snapshot_importer_handoff'] and observed['capability_mismatch_rejected'] and observed['memory_refused_before_payload'] and observed['near_limit_peak_bytes']>observed['receive_allocated_bytes']==8192 and 0<observed['receive_buffered_high_water_bytes']<=observed['receive_allocated_bytes'] and observed['receive_complete_frames_high_water']>1 and observed['receive_bound_enforced'] and not observed['shared_frame_backing'] and observed['exact_limit_payload_fetches']==1
else:
...
fp=sha(canon(cfg));before={'generation_state':'copying','pending_chunks':3,'active_workers':0};after={k:observed[k] for k in ('generation_state','bounded_reader_released','atomic_chunk_event_commit')};after.update({k:observed[k] for k in ('stale_completion_rejected','snapshot_events','complete_chunks','remaining_claims','exported_snapshot_importer_handoff','capability_mismatch_rejected','memory_refused_before_payload','near_limit_peak_bytes','exact_limit_payload_fetches','receive_allocated_bytes','receive_buffered_high_water_bytes','receive_complete_frames_high_water','receive_bound_enforced','shared_frame_backing') if k in observed});after['keyset_only']=observed.get('postgres_keyset_runs')==2
