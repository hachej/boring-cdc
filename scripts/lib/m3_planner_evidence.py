...
files=['src/m3_bootstrap.rs','src/m3_planner.rs','src/lib.rs','contracts/m3/planner-cases.json','scripts/e2e/m3_planner.sh','scripts/faults/m3_planner.sh','scripts/lib/m3_planner_evidence.py']
h=hashlib.sha256()
for n in files:
...
if mode=='e2e':
 assert observed['generation_state']=='fencing' and observed['snapshot_events']==5 and observed['complete_chunks']==3 and observed['remaining_claims']==0 and observed['rust_worker_runs']==2 and observed['postgres_keyset_runs']==2 and observed['exported_snapshot_importer_handoff'] and observed['capability_mismatch_rejected'] and observed['memory_refused_before_payload'] and observed['near_limit_peak_bytes']>1 and observed['exact_limit_payload_fetches']==1
else:
