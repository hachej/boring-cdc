...
if mode=='e2e':
 assert observed['generation_state']=='fencing' and observed['snapshot_events']==5 and observed['complete_chunks']==3 and observed['remaining_claims']==0 and observed['rust_worker_runs']==2 and observed['postgres_keyset_runs']==2 and observed['exported_snapshot_importer_handoff'] and observed['memory_refused_before_payload']
else:
...
fp=sha(canon(cfg));before={'generation_state':'copying','pending_chunks':3,'active_workers':0};after={k:observed[k] for k in ('generation_state','bounded_reader_released','atomic_chunk_event_commit')};after.update({k:observed[k] for k in ('stale_completion_rejected','snapshot_events','complete_chunks','remaining_claims','exported_snapshot_importer_handoff','memory_refused_before_payload') if k in observed});after['keyset_only']=observed.get('postgres_keyset_runs')==2
