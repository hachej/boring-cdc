...
def execute(out:Path)->None:
 ...
 source_digest=source_snapshot();head=git("rev-parse","HEAD");suffix=head[:12];project=f"boring-cdc-m0-scaffold-{suffix}";builder=f"m0-scaffold-{suffix}";image=f"boring-cdc-connector:{suffix}"
 ...
 with tempfile.TemporaryDirectory(prefix="m0-scaffold-",dir=os.environ.get("TMPDIR","/var/tmp")) as td:
  ...
  try:
   ...
   clean_env={"DOCKER_CONFIG":str(cli_config),"HOME":str(proof_root),"LC_ALL":"C","PATH":"/usr/bin:/bin","SOURCE_DATE_EPOCH":"0","TZ":"UTC"}
