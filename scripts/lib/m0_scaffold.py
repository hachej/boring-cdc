...
PROVISIONAL_AUTHORITIES = {
 ...
 "fixtures/m0/decisions/boring-cdc-d-values.json":set(),
 "fixtures/m0/decisions/boring-cdc-d-wal-cap.json":{"boring-cdc-d-wal-cap"},
 "contracts/archive/archive-model.json":{"boring-cdc-d-compose"},
 "contracts/archive/archive-model.schema.json":{"boring-cdc-d-compose"},
 "contracts/clickhouse/model.json":{"boring-cdc-d-compose","boring-cdc-d-keys"},
 "contracts/event/event-format.json":{"boring-cdc-d-keys"},
 "contracts/storage/sqlite-schema.sql":{"boring-cdc-d-sqlite"},
 "contracts/storage/storage-model.json":{"boring-cdc-d-admission","boring-cdc-d-compose","boring-cdc-d-sqlite","boring-cdc-d-wal-cap"},
 "docs/CLICKHOUSE_MODEL.md":{"boring-cdc-d-compose","boring-cdc-d-keys"},
 "docs/EVENT_FORMAT.md":{"boring-cdc-d-keys"},
 "scripts/lib/core_validator.py":{"boring-cdc-d-compose","boring-cdc-d-sqlite","boring-cdc-d-wal-cap"},
 "scripts/validate/archive_contract.py":{"boring-cdc-d-compose"},
 "scripts/validate/compose_spec.sh":{"boring-cdc-d-compose"},
 "scripts/validate/sqlite_durability.sh":{"boring-cdc-d-sqlite"},
 "scripts/validate/supported_values.sh":set(),
 "scripts/validate/wal_cap.sh":{"boring-cdc-d-wal-cap"},
 "scripts/validate/clickhouse_contract.py":{"boring-cdc-d-compose","boring-cdc-d-keys"},
 "scripts/validate/event_format.py":{"boring-cdc-d-keys"},
 "scripts/validate/storage_contract.py":{"boring-cdc-d-admission","boring-cdc-d-compose","boring-cdc-d-sqlite","boring-cdc-d-wal-cap"},
 "tests/test_archive_contract.py":{"boring-cdc-d-compose"},
 "tests/test_clickhouse_contract.py":{"boring-cdc-d-compose","boring-cdc-d-keys"},
 "tests/test_event_format.py":{"boring-cdc-d-keys"},
 "tests/test_storage_contract.py":{"boring-cdc-d-admission","boring-cdc-d-compose","boring-cdc-d-sqlite","boring-cdc-d-wal-cap"},
}
...
def isolated_clean_environment(docker_config:Path, home:Path, source_date_epoch:str, ambient:dict[str,str]|None=None)->dict[str,str]:
 ...
 del ambient
 result={"DOCKER_CONFIG":str(docker_config),"HOME":str(home),"LC_ALL":"C","PATH":"/usr/bin:/bin","SOURCE_DATE_EPOCH":source_date_epoch,"TZ":"UTC"}
 if set(result)!=CLEAN_ENVIRONMENT_KEYS:raise RuntimeError("clean environment inventory changed")
 return result
...
def execute(out:Path)->None:
 ...
 with tempfile.TemporaryDirectory(prefix="m0-scaffold-",dir="/var/tmp") as td:
  ...
  try:
   ...
   # registry resolution. Build/Compose receive only explicit isolated runtime inputs.
   env={**clean_env,"DOCKER_HOST":f"unix://{socket}","COMPOSE_PROJECT_NAME":project,"BORING_CDC_CONNECTOR_IMAGE":image,"BORING_CDC_POSTGRES_PASSWORD_FILE":str(proof_root/"postgres_password")};(proof_root/"postgres_password").write_text("isolated-scaffold-fixture\n");(proof_root/"postgres_password").chmod(0o600)
   ...
   for attempt in range(2):
    ...
    try:
     ...
     common.append(run_record(["python3","scripts/validate/compose_manifests.py"],proof,clean_env))
     ...
     common.append(run_record(["docker","buildx","build","--builder",builder,"--platform","linux/amd64","--build-arg",f"SOURCE_DATE_EPOCH={author_timestamp}","--output",f"type=docker,name={image},rewrite-timestamp=true","."],proof,env))
