...
def main():
 ...
 for hook in ['after-intent','after-write','after-file-sync','after-directory-sync','after-rename','after-parent-sync','before-marker','after-marker-write','after-marker-sync','before-checkpoint']:
  ...
  recover_out=out/'process'/f'{hook}-recover.stdout';recover_err=out/'process'/f'{hook}-recover.stderr';w(recover_out,recovered.stdout);w(recover_err,recovered.stderr);extra_commands.append((f'env BORING_CDC_JSONL_PROCESS_BASE={base} BORING_CDC_JSONL_PROCESS_OBSERVATION={observed} BORING_CDC_JSONL_PROCESS_PHASE=recover cargo test --quiet --locked {probe} -- --exact --nocapture',recover_out,recover_err,recovered.returncode))
