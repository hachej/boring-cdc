...
contract_sha='71de4619782c2d11dedf509eaf40845810f7e6d58c40e309e3fb13c9d4b1cb5a'; result_schema_rel='contracts/m0/compose-execution-result.schema.json'; result_schema_sha='90e27ad47c1381fe32164886865bfa2fd0789537c2dd5dacab4d75fe6ea617f4'
executors=['boring-cdc-m0-scaffold','boring-cdc-m6-failure-matrix','boring-cdc-m7-artifacts']
markers=[]
proposed='linux/amd64 only; PostgreSQL docker.io/library/postgres:17.6 index sha256:00bc86618629af00d2937fdc5a5d63db3ff8450acf52f0636ec813c7f4902929 and amd64 sha256:b86568d3e0fe1dfaeff52714f9da36f206a30e4c49131b82bf96982d78627409; ClickHouse docker.io/clickhouse/clickhouse-server:25.8.2.29 index sha256:74c213b4d4cb4854c2497694df0c2d153c041003eadbb0457ae62c28cb8d723f and amd64 sha256:78b6f0863688458b229b597f6a1bbf891855a01cf59c3f3dbe66428571c518c9; builder rust:1.89.0-bookworm index sha256:948f9b08a66e7fe01b03a98ef1c7568292e07ec2e4fe90d88c07bb14563c84ff and amd64 sha256:c9ac3fa8945b61dede1e4500d25028aa8fd8a8fe46365fcf9c0422f8d999b9b0; runtime debian:bookworm-20250811-slim index sha256:b1a741487078b369e78119849663d7f1a5341ef2768798f7b7406c4240f86aef and amd64 sha256:cea2634840f5a87503d8210e4df97b9f23a2acd67ff860a76c133d963032f866; Docker Engine 28.3.3, Compose 2.39.2, BuildKit 0.24.0, Dockerfile frontend 1.12.0; cargo build --locked --release; project boring-cdc with postgres, clickhouse, connector and pgdata, chdata, cdcdata; restart unless-stopped with persisted FailurePolicy retry authority; PostgreSQL keepalive 30/10/3 and client check 10 seconds, reap <=70 seconds, takeover 90 seconds; health 5/3/12 with 30-second start and 120-second readiness; patch/minor clean-pull with rollback-compatible store; PostgreSQL/ClickHouse major migration plan and reseed.'
...
try:
 ...
 required=('inputs','preconditions','supported_matrix','deterministic_phase','expected','expected_failure','result_contract','redaction_assertions','later_executors','vectors','confirmed_boundary','approval')
 ensure(all(spec.get(x) for x in required) and spec['confirmed_boundary']==contract and spec['later_executors']==executors)
 ensure(spec['deterministic_phase']=='validate approved owner-card literals, ownership, vectors, hashes and expected transitions without pulling images or running services')
 approval={'approved_at':'2026-09-16T06:05:55.752Z','approved_by':'Julien Hurault (repository owner)','intention_id':'59a63169-9b07-4a00-bbfb-cbed02a7e4ac','selection':'Accept recommended defaults'}
 ensure(spec['approval']==approval and contract.get('approval')==approval and spec['preconditions']==['answered owner card 59a63169 accepts the exact recommended Compose defaults','all inputs and artifact hashes resolve','runtime results are not an M0 prerequisite'])
 ...
 ensure(len(clean.get('manifest_verification',[]))==4 and {x['image_key'] for x in clean['manifest_verification']}==set(expected_images))
 for item in clean['manifest_verification']:
  repo,tag,index,platform=expected_images[item['image_key']]; ensure(item=={'image_key':item['image_key'],'index_command':['docker','buildx','imagetools','inspect','--raw',repo+'@sha256:'+index],'index_digest':'sha256:'+index,'platform':'linux/amd64','platform_descriptor_digest':'sha256:'+platform,'platform_command':['docker','buildx','imagetools','inspect','--raw',repo+'@sha256:'+platform]})
 ensure('exact raw OCI index bytes' in clean['digest_verification'] and 'exits 78 before build' in clean['digest_verification'])
 ensure(clean.get('manifest_validator')=={'path':'scripts/validate/compose_manifests.py','sha256':sha(root/'scripts/validate/compose_manifests.py')})
 ...
 required_fields={'images','tool_versions','configuration_sha256','cargo_lock_sha256','dockerfile_sha256','build_recipe_sha256','start_times','readiness_times','health_outputs','exit_codes'}
 ...
 ensure(vectors['dependency_delay']['fault_hook']=='postgres dependency readiness wait' and vectors['dependency_delay']['inputs']=={'deadline_seconds':120,'failure_class':'transient_source','failure_exit_class':'temporary_dependency','component':'source','domain':'postgresql','ready_at_seconds':121} and vectors['dependency_delay']['inputs']['failure_class'] in failure['failure_classes'])
