...
reg_got=hashlib.sha256(open(root/'contracts/agent/stable-ids.json','rb').read()).hexdigest()
if got!=prov.get('generated_digest') or prov.get('source_digests',{}).get('contracts/agent/stable-ids.json')!=reg_got:f.append(['E_GENERATED_VIEW_DRIFT','contracts/coverage/plan-to-beads.json'])
