...
EXPECTED_CONTRACT_SHA256='3205cfe18758f6bf5f4a719a4f66b1b23ee35766198e2425761066fc91276306'
...
def validate():
 ...
 if set(c.get('provisional_markers',[]))!=markers:add(fs,'E_PROVISIONAL','provisional_markers','only unrelated admission marker may remain')
 if set(c['admission'].get('provisional',[]))!={marker('boring-cdc-d-admission')}:add(fs,'E_PROVISIONAL','admission/provisional','only admission marker may remain')
 if c['writer_service'].get('provisional')!=marker('boring-cdc-d-admission'):add(fs,'E_PROVISIONAL','writer_service/provisional','writer admission marker missing')
 if c['authority']['owner_cards']!=['59a63169'] or not c['authority']['status'].startswith('owner-accepted'):add(fs,'E_PROVISIONAL','authority','owner card 59a63169 acceptance missing')
 ...
 if hashlib.sha256(C.read_bytes()).hexdigest()!=EXPECTED_CONTRACT_SHA256:add(fs,'E_CONTRACT_DIGEST','storage-model.json','entire provisional contract changed without validator reconciliation')
