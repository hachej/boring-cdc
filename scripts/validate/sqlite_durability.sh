...
markers=[]
proposed='SQLite 3.45.3; one physical database with logically append-only journal_events; WAL; synchronous=FULL on every admitted durability connection; journal_size_limit=256 MiB; page_size=4096; temp_store=FILE; foreign_keys=ON; secure_delete=FAST; pre-schema auto_vacuum=INCREMENTAL; wal_autocheckpoint=0; busy_timeout=5000 ms; mmap_size=0; at most 16 readers, each transaction at most 5,000 ms and 4,096 pages; local ext4/XFS non-network block devices only; PASSIVE checkpoint at 1,000 WAL pages or 30 seconds, RESTART only when pins permit, TRUNCATE maintenance-only; incremental vacuum at most 1,000 pages per cycle; attestation valid 24 hours and invalid on mount/device change; FULL sync of database/WAL and required directory lifecycle; quick_check at startup and daily, integrity_check maintenance-only; SHA-256 transaction checksums and exported evidence/backups; no automatic full VACUUM.'
...
try:
 ...
 required=('approval','confirmed_boundary','inputs','preconditions','supported_matrix','deterministic_phase','expected','expected_failure','result_contract','redaction_assertions','later_executors','vectors')
 if any(not spec.get(x) for x in required): fail()
 accepted_payload={k:spec[k] for k in ['approval', 'confirmed_boundary', 'connection_admission', 'directory_sync', 'fixed_seed', 'inputs', 'preconditions', 'supported_matrix', 'vectors', 'later_executors']}
 if hashlib.sha256(json.dumps(accepted_payload,sort_keys=True,separators=(',',':')).encode()).hexdigest()!='8fde90f4f89f160d077f4679b349ba9950287b685527842da5dce336a28d72e8': fail()
 ...
 if spec.get('approval')!={'approved_at':'2026-09-16T06:05:55.752Z','approved_by':'Julien Hurault (repository owner)','intention_id':'59a63169-9b07-4a00-bbfb-cbed02a7e4ac','selection':'Accept recommended defaults'} or spec.get('provisional_markers',[])!=markers: fail()
 expected_boundary={'sqlite_version':'3.45.3','database_count':1,'payload_journal':'journal_events_logically_append_only','pragmas':{'journal_mode':'WAL','synchronous':'FULL','journal_size_limit_bytes':268435456,'page_size_bytes':4096,'temp_store':'FILE','foreign_keys':'ON','secure_delete':'FAST','auto_vacuum':'INCREMENTAL','wal_autocheckpoint_pages':0,'busy_timeout_ms':5000,'mmap_size_bytes':0},'reader_limits':{'max_transaction_age_ms':5000,'max_pages_per_transaction':4096,'max_readers':16},'filesystems':{'allow':['ext4','xfs'],'device':'local_non_network_block_device','reject':['nfs','smb_cifs','fuse','tmpfs','overlayfs','remote_volume']},'checkpoint':{'passive_wal_pages':1000,'passive_interval_seconds':30,'restart':'only_when_no_pin_blocks_recycling','truncate':'maintenance_only'},'incremental_vacuum_pages_per_cycle_max':1000,'automatic_full_vacuum':'prohibited','attestation':{'validity_seconds':86400,'invalidate_on':['mount_change','device_change']},'integrity':{'quick_check':['startup','daily'],'integrity_check':'maintenance_only','exported_evidence_backup_digest':'SHA-256'},'permissions':{'directory_mode':'0700','database_wal_and_related_file_mode':'0600_or_stricter'},'crash_claim':{'guarantees':['process_crash','container_crash','abrupt_host_restart_on_honest_tested_storage_stack'],'does_not_guarantee':['media_or_controller_failure','filesystem_corruption','hardware_lying_about_flushes']}}
 ...
 required_cases={'ext4_admitted','xfs_admitted','unsupported_filesystem','writer_off_observer_full','reopened_writer_full','stale_attestation','mount_or_device_change','reader_limit','reader_age_limit','reader_range_limit','passive_page_threshold','passive_time_threshold','wal_busy_reader_pin','restart_pin_permitted','truncate_runtime_forbidden','incremental_vacuum_bound','checksum_mismatch','abrupt_host_ext4','abrupt_host_xfs','directory_publication_sync'}
 ...
 if vectors['writer_off_observer_full']['executor_bead']!='boring-cdc-m2-schema' or vectors['writer_off_observer_full']['inputs']!={'actual_writer_synchronous':'OFF','observer_synchronous':'FULL'} or vectors['writer_off_observer_full']['expected']['feedback']!='none': fail()
 ...
 if vectors['reader_age_limit']['inputs']!={'reader_transaction_age_ms':5001,'max_transaction_age_ms':5000} or vectors['reader_range_limit']['inputs']!={'reader_transaction_pages':4097,'max_pages_per_transaction':4096}: fail()
 ...
 expected_probe=[{'code':'SQLITE_DURABILITY_FIXTURE_VALID','outcome':'pass','phase':'validate_spec','vector_count':20}]
 ...
 approval={'approved_at':'2026-09-16T06:05:55.752Z','approved_by':'Julien Hurault (repository owner), answered owner card 59a63169','value_digest':hashlib.sha256(proposed.encode()).hexdigest()}
