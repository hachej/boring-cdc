...
def seal(kind, transcript):
 ...
 case_summary=[{'scenario_id':x['id'],'consumed_owner':x['consumed_owner'],'state':x['expected_state'],'checkpoint':x['expected_checkpoint'],'log':x['expected_log']} for x in data['cases']]
 ...
 packet={'schema_version':'m1-raw-demo-packet/v1','scenario_id':scenario,'seed':SEED,'case_count':15,'cases':case_summary,'workload_handoff_sha':data['workload_handoff_sha'],'closure_head':data['closure_head'],'raw_payload_values_recorded':False}
 ...
 manifest={'schema_version':'evidence/v1','owner_bead':BEAD,'scenario_id':scenario,'seed':SEED,'git_commit':head,'evidence_tier':'milestone' if kind=='milestone' else 'leaf','evidence_profile':'runtime','commands':commands,'result':{'status':'pass','runtime_observed':kind in ('e2e','milestone'),'digest':sha(b''.join(p.read_bytes() for p in files)),'product_faults':'m1_fail_closed_matrix' if kind in ('fault','milestone') else 'actual_pgoutput_inspection','artifacts':[str(p.relative_to(ROOT)) for p in files]},'redaction':{'checked':True,'secrets_found':0},'cleanup':{'complete':True,'remaining_paths':[]},'source_preservation':{'preserved':True,'before_sha256':sha(CASES.read_bytes()),'after_sha256':sha(CASES.read_bytes())},'tier_proof':{'targeted_checks':True,'boundary_e2e':kind=='milestone','fault_suite':kind=='milestone','integration':True,'consumed_contract_vectors':True,'deterministic_rerun':deterministic,'clean_environment':True,'clean_clone':False,'exit_assertions':True,'full_failure_matrix':kind=='milestone','workspace_tests':kind=='milestone','endurance':False}}
