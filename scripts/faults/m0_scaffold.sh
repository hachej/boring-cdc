...
trap 'rm -f "$tmp" "${tmp}.status"' EXIT HUP INT TERM
...
mismatch=$?
printf '%s\n' '{"Service":"postgres","Health":"unhealthy","State":"running"}' > "${tmp}.status"
python3 scripts/lib/m0_scaffold.py probe SCN-M0-SCAFFOLD-DEPENDENCY-DELAY --path "${tmp}.status" >/dev/null
delay=$?
set -e
