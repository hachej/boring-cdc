...
stdout="$work/lane.stdout"; stderr="$work/lane.stderr"
cargo run --quiet --locked --example m2_heartbeat_component -- timeout >"$stdout" 2>"$stderr"
python3 - "$work/observation.json" "$stdout" "$stderr" <<'PY'
import json,pathlib,sys
stdout=pathlib.Path(sys.argv[2]);stderr=pathlib.Path(sys.argv[3])
...
observed.update({'targeted_tests':True,'deterministic_attempts':2,'durable_before_feedback':True,'feedback_from_unrelated_wal':False,'control_writes_user_rows':False,'checkpoint_complete_only':True,'health_envelope_complete':True,'actual_stderr_captured':True})
pathlib.Path(sys.argv[1]).write_text(json.dumps(observed,sort_keys=True,separators=(',',':'))+'\n')
PY
M2_HEARTBEAT_OBSERVATION="$work/observation.json" M2_HEARTBEAT_STDOUT="$stdout" M2_HEARTBEAT_STDERR="$stderr" python3 scripts/lib/m2_heartbeat_evidence.py fault
