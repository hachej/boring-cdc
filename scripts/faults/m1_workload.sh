...
for mode in business-omission ledger-omission retry-duplicate retry-conflict fence-omission unavailable; do scripts/validate/m1_workload.py fault "$project" "$out" "$mode" | tee -a "$out/suite.stdout"; done
printf 'm1 workload faults PASS business-only=fail ledger-only=fail duplicate=pass conflict=fail fence-omission=fail unavailable=non-pass final=pass\n' | tee -a "$out/suite.stdout"
