...
CREATE TABLE boring_cdc_control.capture_fences(
 id text PRIMARY KEY CHECK(id='singleton'), capture_epoch text NOT NULL,
 generation bigint NOT NULL, table_set_fingerprint text NOT NULL, unique_nonce text NOT NULL);
INSERT INTO boring_cdc_control.capture_fences VALUES('singleton','epoch',1,'tables','seed');
...
SQL
  update_result=$("${psql_cmd[@]}" -c "UPDATE boring_cdc_control.capture_fences SET capture_epoch='epoch',generation=1,table_set_fingerprint='tables',unique_nonce='nonce-1' WHERE id='singleton'")
...
assert commits and any(b'nonce-1' in data for _,_,data in rows)
commit=commits[-1]
