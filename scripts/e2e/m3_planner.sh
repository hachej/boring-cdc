...
o=runs[0];expected={'atomic_chunk_event_commit':True,'capability_mismatch_rejected':True,'complete_chunks':3,'exact_limit_payload_fetches':1,'exported_snapshot_importer_handoff':True,'generation_state':'fencing','memory_refused_before_payload':True,'pending_before':3,'remaining_claims':0,'snapshot_events':5,'receive_allocated_bytes':8192,'shared_frame_backing':False};assert all(o.get(k)==v for k,v in expected.items());assert o['near_limit_peak_bytes']>o['receive_allocated_bytes'];assert 0<o['receive_buffered_high_water_bytes']<=o['receive_allocated_bytes'];assert o['receive_complete_frames_high_water']>1
o.update(row_count=len(b.splitlines()),result_sha256=hashlib.sha256(b).hexdigest(),rust_worker_runs=len(runs),postgres_keyset_runs=2,bounded_reader_released=o['pending_before']==3 and o['complete_chunks']==3,limits_respected=o['snapshot_events']==5,receive_bound_enforced=True)
