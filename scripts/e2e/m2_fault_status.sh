...
# restart reconciliation, and distinct missing/unreserved/lost slot states.
...
assert a['overall_health']=='blocked' and len(a['conditions'])>=2
assert all(c['procedure_status']=='pending_m6' and c['runbook_id'] for c in a['conditions'])
...
assert a['boundaries']['feedback_lsn'] is None
PY
