...
"#;
const IMPORT_IDENTITY_TRIGGER_SQL: &str = r#"
CREATE TRIGGER IF NOT EXISTS m3_fence_import_identity_immutable BEFORE UPDATE ON m3_fence_intents
 WHEN NEW.import_id IS NOT OLD.import_id OR NEW.destination_id IS NOT OLD.destination_id
   OR NEW.capture_epoch IS NOT OLD.capture_epoch OR NEW.generation IS NOT OLD.generation
...
pub fn install(connection: &Connection) -> Result<(), FenceError> {
    ...
    let mut statement =
        connection.prepare("SELECT name FROM pragma_table_info('m3_fence_intents')")?;
    let columns = statement
        .query_map([], |row| row.get::<_, String>(0))?
        .collect::<Result<Vec<_>, _>>()?;
    drop(statement);
    for (name, definition) in [
        ("import_id", "TEXT REFERENCES bootstrap_imports(import_id)"),
        (
            "destination_id",
            "TEXT REFERENCES destinations(destination_id)",
        ),
        ("configuration_fingerprint", "TEXT"),
    ] {
        if !columns.iter().any(|column| column == name) {
            connection.execute_batch(&format!(
                "ALTER TABLE m3_fence_intents ADD COLUMN {name} {definition}"
            ))?;
        }
    }
    connection.execute_batch(IMPORT_IDENTITY_TRIGGER_SQL)?;
    Ok(())
}
...
mod tests {
    ...
    fn setup(name: &str) -> FenceStore {
        let w = writer(name);
        w.connection().execute_batch("CREATE TABLE m3_bootstrap_runtime(intent_id TEXT PRIMARY KEY,generation INTEGER,start_seq INTEGER,consistent_lsn TEXT,table_set_fingerprint TEXT,state TEXT,exporter_liveness TEXT,guard_liveness TEXT,snapshot_promotable INTEGER,feedback_gate_open INTEGER,revision INTEGER,configuration_fingerprint TEXT); CREATE TABLE m3_bootstrap_importers(intent_id TEXT,importer_id TEXT,state TEXT,revision INTEGER,snapshot_schema_fingerprint TEXT,PRIMARY KEY(intent_id,importer_id)); CREATE TABLE m3_planner_runs(generation_id TEXT,bootstrap_intent_id TEXT,next_snapshot_seq INTEGER); CREATE TABLE m3_canonical_import_proofs(import_id TEXT PRIMARY KEY,destination_id TEXT,capture_epoch TEXT,generation INTEGER,configuration_fingerprint TEXT); CREATE TABLE m3_chunk_claims(generation_id TEXT);").unwrap();
        install(w.connection()).unwrap();
        w.connection().execute_batch("INSERT INTO destinations(destination_id,kind,configuration_fingerprint,capture_epoch,generation) VALUES('d','archive','cfg','1',1); INSERT INTO bootstrap_intents VALUES('boot','1','sys','db','slot','0000000000000001','slot_created',0,'now'); INSERT INTO m3_bootstrap_runtime VALUES('boot',1,0,'0000000000000001','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','exporter_released','released','held',1,1,0,'cfg'); INSERT INTO m3_bootstrap_importers VALUES('boot','worker','acknowledged',0,'schema'); INSERT INTO m3_feedback_gates VALUES('boot',1,'open',NULL,0); INSERT INTO bootstrap_imports VALUES('import','boot','d','acknowledged',0); INSERT INTO m3_canonical_import_proofs VALUES('import','d','1',1,'cfg'); INSERT INTO backfill_runs VALUES('run','d','1','running',0); INSERT INTO backfill_generations VALUES('gen','run',1,NULL,'fencing'); INSERT INTO backfill_chunks VALUES('chunk','gen',X'',X'','complete',5,'sum'); INSERT INTO m3_planner_runs VALUES('gen','boot',5); INSERT INTO relation_schemas VALUES('schema','1','rel',X'00','sum',1);").unwrap();
        FenceStore::open(w).unwrap()
    }
    #[test]
    fn install_upgrades_legacy_fence_intent_schema_before_creating_identity_trigger() {
        let w = writer("legacy-intent-schema");
        w.connection().execute_batch(
            "CREATE TABLE m3_bootstrap_runtime(intent_id TEXT PRIMARY KEY);
             CREATE TABLE m3_fence_intents(
              intent_id TEXT PRIMARY KEY, generation_id TEXT NOT NULL UNIQUE REFERENCES backfill_generations(generation_id),
              bootstrap_intent_id TEXT NOT NULL REFERENCES m3_bootstrap_runtime(intent_id), capture_epoch TEXT NOT NULL,
        ...
        install(w.connection()).unwrap();
        for column in ["import_id", "destination_id", "configuration_fingerprint"] {
            assert_eq!(
                w.connection()
                    .query_row(
                        "SELECT count(*) FROM pragma_table_info('m3_fence_intents') WHERE name=?1",
                        [column],
                        |row| row.get::<_, i64>(0),
                    )
                    .unwrap(),
        ...
        assert_eq!(
            w.connection()
                .query_row(
                    "SELECT count(*) FROM sqlite_schema WHERE type='trigger' AND name='m3_fence_import_identity_immutable'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    fn acknowledged_bootstrap_planner_and_fence_compose_one_canonical_anchor() {
        ...
        fence.writer.connection().execute(
            "UPDATE destinations SET generation=2,revision=revision+1 WHERE destination_id='destination'",
            [],
        ).unwrap();
        assert!(matches!(
            fence.prepare_after_copy(&input()),
            Err(FenceError::Conflict("M3_FENCE_PREREQUISITE_MISSING"))
        ));
        fence.writer.connection().execute(
            "UPDATE destinations SET generation=1,revision=revision+1 WHERE destination_id='destination'",
            [],
        ).unwrap();
        fence.writer.connection().execute(
            "UPDATE destinations SET configuration_fingerprint='post-plan-drift',revision=revision+1 WHERE destination_id='destination'",
            [],
        ).unwrap();
        assert!(matches!(
            fence.prepare_after_copy(&input()),
        ...
        fence.writer.connection().execute(
            "UPDATE destinations SET configuration_fingerprint='post-fence-drift',revision=revision+1 WHERE destination_id='destination'",
            [],
        ).unwrap();
        assert!(matches!(
            fence.observe_durable("intent", "tx"),
        ...
        assert!(
            fence
                ...
                .connection()
                .execute(
                    "UPDATE m3_canonical_import_proofs SET configuration_fingerprint='mutated'",
                    [],
                )
                .is_err()
        );
        ...
        s.writer
            ...
            .execute_batch(
                "UPDATE m3_canonical_import_proofs SET generation=2,configuration_fingerprint='coordinated';
                 UPDATE m3_bootstrap_runtime SET generation=2,configuration_fingerprint='coordinated',table_set_fingerprint='bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' WHERE intent_id='boot';
                 UPDATE destinations SET generation=2,configuration_fingerprint='coordinated',revision=revision+1 WHERE destination_id='d';
                 UPDATE backfill_generations SET generation=2,intended_fence_nonce='coordinated' WHERE generation_id='gen';",
            )
            .unwrap();
        assert!(matches!(
