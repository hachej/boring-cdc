...
//!
//! The module deliberately treats canonical keys and typed row values as opaque bytes whose
//! contract digests are supplied by the caller. It does not choose PostgreSQL types, normalize
//! values, or claim that an unavailable provider event boundary passed.
...
/// Builds the fixed provider-neutral corpus. Hash bytes are fixture tokens only; production
/// adapters must supply hashes created under the three consumed contract digests.
