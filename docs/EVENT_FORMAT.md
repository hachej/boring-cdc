...
`contracts/event/event-format.json#value_types` is the admitted v1 matrix. The non-float value matrix, encodings, mappings, and limits are accepted under owner card `59a63169`. Canonical float spelling remains separately provisional under `// M0-PROVISIONAL: boring-cdc-d-values.1`. It freezes booleans, signed integers, OID, IEEE floats, arbitrary-precision numeric, date, timestamp, timestamptz, UUID, UTF-8 text-family values, bytea, and one-dimensional arrays of admitted non-array scalars. Type OID and typmod remain part of a value's identity; no silent coercion, locale collation, Unicode normalization, timezone reinterpretation, or JSON-number conversion is allowed.
...
Owner card `59a63169` accepted the d-values literals materialized here. Canonical float spelling (`boring-cdc-d-values.1`) and key literals remain separately provisional, and decision Bead state remains separately governed.
