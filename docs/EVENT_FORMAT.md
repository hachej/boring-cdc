...
`contracts/event/event-format.json#value_types` is the admitted v1 matrix. The value matrix, encodings, mappings, limits, and corrected globally shortest round-trip float spelling are accepted under owner card `59a63169`. It freezes booleans, signed integers, OID, IEEE floats, arbitrary-precision numeric, date, timestamp, timestamptz, UUID, UTF-8 text-family values, bytea, and one-dimensional arrays of admitted non-array scalars. Type OID and typmod remain part of a value's identity; no silent coercion, locale collation, Unicode normalization, timezone reinterpretation, or JSON-number conversion is allowed.
...
A WAL event ID hashes domain `boring-cdc/wal-event/v1` and `(capture_epoch u64, source_slot_identity 32 bytes, transaction_end_lsn u64, row_ordinal u64, mutation_ordinal u8)`. `source_slot_identity` hashes, in order, system identifier u64, timeline u32, and UTF-8 database identity, slot name, and plugin under `boring-cdc/source-slot/v1`. A snapshot event ID hashes `boring-cdc/snapshot-event/v1` and `(capture_epoch u64, generation u64, logical_table_id 32 bytes, chunk_id u64, canonical_key)`. The canonical key is encoding-version byte `1`, one-byte component count, then each component in effective replica-identity index order as u32-be PostgreSQL type OID, u32-be payload length, and canonical payload bytes; the physical `key_hash` is not an identity input. Payload bytes, journal sequence, run ID, wall time, ingest time, cache timing, retries, and destination state are excluded from positional identity.
...
Canonical row correctness identity is exactly `(capture_epoch, logical_table_id, canonical_key)`. It is distinct from relation/schema identity and remains the grouping key for snapshot keysets, WAL update/delete identity, checksums, and destination state. `key_hash` hashes the same typed, length-safe effective replica-identity encoding under `boring-cdc/physical-key/v1`; it is only a physical sorting/sharding aid and cannot replace or establish correctness identity. Owner card `59a63169` accepts int2/int4/int8, bounded numeric, UUID, date, timestamp, timestamptz, C-collated text/varchar/bpchar, and bytea key components; arity is 1..8 and each component is at most 1,024 bytes. Empty text and bytea values are valid components; zero-component keys, null, absent, partial, oversized, or unchanged-TOAST states are forbidden. PostgreSQL `bpchar` ordering ignores trailing spaces even though canonical payload bytes preserve the original value.
...
Owner card `59a63169` accepted the d-values literals, corrected canonical float spelling, and d-keys literals materialized here. Decision Bead state remains separately governed.
