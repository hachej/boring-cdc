...
Inclusive limits are scalar 1,048,576 bytes, reconstructed row 4,194,304 bytes, and complete event 8,388,608 bytes. `// M0-PROVISIONAL: boring-cdc-d-values`
...
`fixtures/m0/event-format/golden-vectors.json` is normative. Exact IDs cover WAL identity, snapshot identity, same-key total order, mutable-key expansion, canonical values, unsupported values, four TOAST/schema states, forbidden key-change TOAST, heartbeat routing, and fence routing. The fixed fixture IDs are:
...
The recommended constants remain provisional engineering inputs until their owner decision Beads reconcile them. The complete authority markers are `// M0-PROVISIONAL: boring-cdc-d-event-id`, `// M0-PROVISIONAL: boring-cdc-d-values`, `// M0-PROVISIONAL: boring-cdc-d-keys`, `// M0-PROVISIONAL: boring-cdc-d-toast`, and `// M0-PROVISIONAL: boring-cdc-d-additive`.
