...
The product exposes one canonical runtime `SystemSnapshot` and every mutating dry-run returns an `ActionPlan` recording its `snapshot_id` and `state_revision` for provenance and binding authorization to explicit action-relevant control revisions. The plan states preconditions, transitions, external effects, resource/continuity consequences, rollback boundary, expected postconditions, and confirmation policy. Confirmation and terminal evidence cite the action-plan digest; changed action-relevant control revisions, fingerprints, or safety preconditions force a new plan; unrelated observations do not. Domain Beads continue to own their transitions—the shared envelopes are not a second state machine.
...
### 3.1 Small typed transition kernel and deterministic execution tests

Use distinct types for received LSN, durable source boundary, journal cursor, source version, capture epoch, and destination generation. Only validated commit/reconciliation paths construct a durable boundary; reading a number from configuration or an informational keepalive cannot manufacture one.

Keep each existing domain's transition logic explicit and testable: current facts plus a typed event produce checked next facts and named effects/intents. Clocks, randomness, and effect completions are injectable. The sole SQLite writer performs persisted compare-and-swap transitions; fixed PostgreSQL, SQLite, ClickHouse, and filesystem adapters perform their effects. This is a small implementation boundary inside the existing binary, not a generic plugin API, workflow engine, or second global state machine.

A bounded deterministic harness drives these same domain transition functions with reordered completions, expiry, cancellation, and crash/restart schedules. It checks durable-before-feedback, invalid-generation exclusion, complete-transaction checkpoints, retention pins, and non-regressing selectors after each step; bounded progress properties additionally assume a fair scheduler and restored resources. Persist seeds and minimize failing traces into regression fixtures. M1 specifies the harness contract, domain milestones add executable cases as their implementations exist, and M6 composes them. Synthetic traces never substitute for real protocol, database, filesystem, or abrupt-host-restart evidence and are not a new pre-M0 runtime gate.
...
A single capture-priority SQLite writer serializes source commits, bounded backfill-chunk commits, destination checkpoint changes, archive intents, and metadata. Capture priority is not strict starvation priority: the scheduler reserves bounded service opportunities for ownership/failure persistence, accepted operator transitions, destination checkpoints, and eligible GC. Requests and queues have bounded sizes, admission rules, maximum waiting targets, and explicit overload outcomes. Backfill/maintenance units retain byte and writer-hold-time bounds. Scheduling occurs between atomic transactions; no source transaction is split or partially acknowledged to meet a latency target.

Blocking SQLite calls, filesystem sync, and compression use explicitly bounded execution capacity separate from ownership probes and standby-status scheduling. While capture waits on spool or commit work, requested status replies can still send only the last already durable safe boundary, under the existing ownership deadline. M0 specifies the scheduler/latency fixtures and their executing owners; M2 measures them under continuous capture, slow storage, and draining consumers. A missed bound triggers its existing fail/stop policy rather than claiming a real-time guarantee over an unresponsive kernel or disk.
...
Destination integrity is checked independently of checkpoint advancement through incremental audits. At startup, periodically, and on `destination verify`, each audit resumes a persisted cursor toward the current complete checkpoint and stops at frozen per-pass byte, event, and wall-time budgets. ClickHouse compares retained journal summaries with batch markers/event identities and recomputes canonical payload hashes from the actual stored correctness-bearing fields, including keys, operation, source version, schema identity, and tagged column values. Comparing a stored `payload_hash` column alone is never verification: the recomputed hash must match both that column and the retained journal hash. These reads and computations count against the same audit budgets. It also fingerprints correctness-critical tables, columns, engines, canonical queries/views, selector schema, and settings. It persists `journal_verified_range`, cursor, freshness-window coverage, evidence digest, and first mismatch. Archive persists two independent coverages: `journal_verified_range` for retained-journal-to-manifest comparison and `self_consistent_range` for selector/manifest/`SEGMENT_READY`/file-hash consistency, each with cursor, budget accounting, and freshness. Coverage is the union of completed passes still inside the frozen freshness window; gaps or older history are explicitly `unverifiable_before_seq`, never silently verified. Audits never read PostgreSQL or mutate a live checkpoint. A mismatch creates the affected destination's `processing_failures` record with reason `integrity_mismatch`; only that destination blocks and recovery requires a fresh verified generation from a compatible anchor, or re-seed when history is unavailable.

Each destination audit proceeds in finite rounds: freeze a complete checkpoint target and the relevant epoch/generation/selector/contract identity, advance bounded units toward that target, then begin a new round over the range whose freshness is promised. Do not chase a moving checkpoint indefinitely or permanently skip previously checked data. Publish the round target, remaining work, and coverage expiry; a target identity change invalidates inapplicable evidence. Preserve the separate journal-verified and self-consistent meanings.

Every indivisible verification unit must fit the admitted byte/event/time pass limits, or have an explicit bounded resumable verification protocol; an oversized Parquet part or transaction cannot cause endless zero-progress passes. Coverage promised within a freshness window must be feasible under measured audit service capacity. If it is not, expose partial/expired/unknown coverage rather than claiming verification; completing a slow round does not refresh already expired subranges. Existing mismatch blocking remains unchanged, and audit scheduling cannot bypass capture/storage safety budgets.
...
### 8.2.1 SQLite reader and checkpoint lifecycle

Every journal, audit, status, and maintenance reader has a bounded read-transaction age and bounded page/range size. Materializers and audits release SQLite statements/read transactions before external I/O, compression, file hashing, or a remote await; bounded copied batches remain inside the existing memory budget. A logical range pin or the existing workflow pin protects still-needed history after the physical SQLite snapshot is released. Such pins have an explicit owner and recovery/release lifecycle; releasing a SQLite reader never authorizes premature GC.

A single maintenance policy owns checkpoint mode, cadence, work limits, busy/progress handling, and automatic-checkpoint configuration on every relevant connection. No unaccounted automatic checkpoint may move unbounded maintenance onto capture's commit path. Export oldest reader age, checkpoint busy/progress, and WAL bytes not yet recyclable. Cancel/release an over-age connector reader before it forces avoidable WAL growth; retain the existing reserve-based safe-stop if cancellation or storage progress cannot be proved. Full startup integrity checks remain mandatory and must be measured against the admitted recovery profile, not silently replaced with sampling.
...
`boring-cdc init` applies database-persistent settings before schema creation. One connection-admission path applies and reads back all required connection-local settings on each actual writer before admitting writes, including reopened and maintenance connections. `journal_mode` and `auto_vacuum` are database-persistent settings; `synchronous` is connection-local. `boring-cdc check` inspects persistent settings read-only without changing the real store. Its own `synchronous` value cannot certify a different connection. Live status exposes fresh writer readback evidence bound to `run_id` and connection generation; offline or stale evidence reports writer-session durability settings as unverified, not healthy. This attestation proves configuration only, not historical durability or honest storage sync semantics. Any filesystem durability/rename/fsync experiment uses a disposable probe under the configured filesystem—not the real state/archive store—and cleans it only after sync/reconciliation. It identifies the mounted filesystem against the M0 allowlist and rejects unknown/unsupported classes. Filesystem API compatibility alone is not a support claim. Weaker settings are available only behind an explicit `unsafe_local_experiment` switch; status is permanently `unsafe_durability`, PostgreSQL feedback remains opt-in, and no durability benchmark/result may be presented as passing.
...
boring-cdc journal inspect [--event-id ID --explain] [--json]
...
The shared CLI registry assigns every command or explicitly distinguished operation variant a stable `CMD-*` ID, canonical handler-owning Bead, mutation/read-only class, ownership requirement, confirmation policy, result schema, exit codes, condition/runbook links, and redaction policy. Base journal inspection and its later explain variant have distinct operation ownership; registering the future variant's schema does not require its runtime handler before its milestone. Help, command inventory, and compatibility fixtures are generated from that registry; command lists in human documents are checked projections rather than independently maintained grammar.

Every mutating dry-run returns a versioned `ActionPlan` with `plan_id`/digest, command ID, observed `SystemSnapshot.snapshot_id` and `state_revision`, bound action-relevant control revisions, asserted preconditions, intended domain transitions, immutable intents and external effects, resource and continuity consequences, affected checkpoints/generations, allowed remaining actions, rollback/recovery boundary, expected postconditions, evidence requirements, expiry, and canonical confirmation argv/token. The lock-owning runtime revalidates the same fields immediately before dispatch. A changed action-relevant control revision or bound fingerprint invalidates the plan. Unrelated checkpoint/telemetry progress may change the observation snapshot without invalidating authorization. Each command registry entry declares its complete control-revision dependency set and current-state safety predicates. The sole writer atomically rechecks ownership, epoch, target generation/configuration, retained-range/anchor availability, expiry, and resource sufficiency as applicable while accepting the intent and its required pins/reservations; external dispatch still performs the existing lease/deadline checks. A changing resource measurement is re-evaluated as a predicate, not ignored or required to remain byte-identical. No global-revision bypass flag is provided. The terminal `CliResult` references the plan digest and persisted postcondition evidence.
...
`journal inspect --event-id ID --explain --json` provides a bounded read-only event explanation using existing positional identities, transaction boundaries, checkpoints, intents, anchors, and generation/selector evidence. It distinguishes locally durable capture, destination checkpoint coverage, current audit coverage, baseline/candidate eligibility, and live-generation selection; these are separate facts, not a single delivered boolean. A destination checkpoint does not prove current payload integrity or current-state winner selection. Report missing/GC-expired evidence as unavailable, avoid PostgreSQL rereads, and do not introduce a second per-event receipt ledger. Return identifiers/hashes and redacted reason codes rather than payloads or credentials by default; use existing indexes and strict read/time budgets. The shared CLI owner specifies the result envelope, while existing domain owners supply their facts as those domains become executable.
...
The ledger is included in the same publication, journal, ClickHouse materialization, and JSONL/Parquet archive as the workload tables. Ledger delivery is not proof that the corresponding business-row event arrived. At each claimed observation boundary, independently enumerate actual business-row events and correlate inserts/updates through their last-mutation fields; correlate deletes using the actual delete operation, canonical key, transaction grouping, and within-transaction multiplicity/order frozen by `boring-cdc-d-oracle`. The workload must retain enough independent event evidence to disambiguate repeated same-key mutations and delete/reinsert/key-change cases; ledger-only inference is forbidden. Internal connector IDs may deduplicate physical retries locally but are not cross-provider identities. PostgreSQL sequence allocation and failed transactions can create gaps, so `mutation_seq` is diagnostic ordering evidence only; no correctness claim assumes a gapless allocator.
...
5. independently reconcile expected post-baseline mutations with actually observed business-row events at each claimed capture/materialization boundary; require complete event-to-ledger correlation, operation/key/payload agreement under the admitted patch semantics, and transaction-local multiplicity, not merely matching ledger rows;
6. compute source and destination keyed counts/checksums from one canonical typed-row serialization; and
7. report separate ledger-delivery, business-event-delivery, and final-state-convergence results, plus duplicate attempts, convergence time, and required operator actions. Missing, unexpected, ambiguous, or mismatched business-event correlation fails the event-delivery result even when the other two results pass.

`canonical_ledger_hash` excludes sequence values/timestamps and uses the frozen provider-neutral canonical fields (`run_id`, `mutation_id`, transaction group, entity table/key, operation, and expected-after hash). Negative fixtures drop only an intermediate business-row update while preserving its ledger entry and a later overwriting update; conversely they drop only the ledger entry while preserving the business event. Both fail their respective proof. Fixtures also cover repeated same-key operations, deletes/reinserts, key-change expansion, legitimate sequence gaps, and physical retry duplicates. Snapshot baseline proof is separate: historical ledger rows copied in a baseline do not prove delivery of pre-baseline business changes. M0 freezes the observation/correlation schema and retained evidence boundary; M1 implements the oracle, M3 integrates snapshot boundaries, and destination owners apply the unchanged oracle to actual history. If a managed destination cannot expose independent intermediate events or sufficient delete correlation, record event-level measurement as `unavailable`, never infer it from a delivered ledger or final state.
...
Each named capacity profile also publishes a recoverability envelope, not just peak throughput. Measure steady incoming journal-equivalent bytes/second, sustainable destination drain rate while capture and normal audits continue, backlog at recovery start, startup/reconciliation time, peak reserved storage, and the first limiting WAL/retention/disk/sink boundary. Normalize drain and incoming rates to the same logical journal-byte units; compressed output bytes are not interchangeable with input bytes.

For approximately stationary admitted rates, report the conditional estimate `catch_up_seconds = backlog_bytes / (drain_bytes_per_second - incoming_bytes_per_second)`. If drain does not exceed arrival, no finite catch-up time is predicted. State the measurement window, workload mix, uncertainty, and assumptions; snapshot/rebuild work needs its own measured rate rather than borrowing steady-state throughput. Integrate backlog/storage growth over outage and catch-up instead of treating the configured replay window as an unconditional outage guarantee.

`status --json` projects this bounded recovery forecast from existing metrics and compatible measured profile evidence, with unavailable/stale inputs explicit. It reports which resource or retention boundary is expected to fail first and points to existing runbooks; it never automatically detaches, drops a slot, or changes limits. M6 tests an outage with positive catch-up headroom and an overload case with no finite catch-up estimate; M7 publishes both, including rejected/unrecoverable profiles and operator steps.
...
| Initial slot creation floor | A persisted floor with no local transaction and null/equal server confirmation cannot bypass slot-invalid/WAL-unavailable rejection; null/equal server-confirmed creation-floor variants never populate durable transaction progress or receive proactive acknowledgement; a `prepared` intent with an existing slot but no persisted floor/token enters `bootstrap_ambiguous_requires_restart`; while its snapshot stays eligible later feedback waits for importer acknowledgements, and atomic invalidation releases the gate for durable WAL draining |
...
| Unexpected CopyBoth loss while advisory lock remains healthy | v0.1 still declares ownership loss, fences dispatch, exits, and resumes only through supervised restart/takeover; in-process replication reopen is a test failure |
...
| Checkpointed ClickHouse row/marker or contract modified externally | Incremental audits recompute hashes from actual stored fields; payload-only corruption preserving IDs/stored hashes/markers is detected. Passes stay within byte/event/time budgets, persist cursor and freshness-window `journal_verified_range`, detect missing/conflicting history or correctness-critical DDL/settings, and block only ClickHouse with replay/rebuild guidance |
...
| Provider-neutral omitted intermediate mutation | Drop only the overwritten business event while retaining its ledger: independent event correlation fails even when ledger sets/final rows match; dropping only the ledger fails ledger delivery. Pre-baseline snapshot history and unavailable managed event visibility are never called event-delivery proof |
...
| Confirmation during normal progress | Unrelated WAL/checkpoint/telemetry updates do not stale a valid action; changed ownership, epoch, target revision/generation, expiry, retained anchor/range, or insufficient reserve rejects before effects |
| Intentional safe-stop transport closure | Persisted capture block precedes expected close; delayed matching EOF does not restart capture, destinations may drain safely, and concurrent advisory-lock loss still fences/exits |
| Writer versus observer SQLite settings | A writer using OFF while a separate read-only observer reports FULL cannot pass writer admission or be certified by the observer; reopened writers reapply/read back settings and stale attestations are unverified |
| Stalled SQLite reader during sustained capture | Bounded reader cancellation/release permits WAL recycling while logical range pins preserve required history; checkpoint contention and reader age are visible and reserves remain intact |
| Saturated writer and slow storage | Continuous capture cannot indefinitely starve failure records, accepted controls, destination checkpoints, or eligible GC; probes and requested feedback remain serviced using only the last durable safe LSN |
| Deterministic cross-domain schedules | The production transition functions preserve safety under reordered invalidation/feedback, safe-stop/ownership-loss, and stale-promotion completions; failing seeds/traces reproduce and shrink |
| Audit progress, freshness, and oversized units | Continued capture cannot prevent a frozen audit round from finishing under admitted service capacity; subsequent rounds revisit promised coverage; oversized work has bounded resume or is rejected, and expired subranges are never renewed by assertion |
| Recovery capacity versus peak throughput | A backlog drains under continuing writes only with measured positive service headroom; overload, missing inputs, and a predicted boundary breach cannot be reported as a finite successful recovery |
| Read-only event explanation | Known, candidate-only, invalid-generation, replayed, and GC-expired events produce distinct evidence-qualified explanations without source reads, payload leakage, live mutations, or unbounded scans |
...
The [series execution schedule](SERIES_EXECUTION.md) maps article claims to actual milestone evidence. `boring-cdc-v01.5` owns early editorial/access preparation without gating M0–M6; M7 remains the owner of final measured external comparisons. Milestone numbers do not imply article-ready live destinations, and no runtime gate may be bypassed to produce an earlier demo.

### Approved robustness revisions — execution ownership

The following nine revisions are approved planning scope, including the event-explanation operation. They do not approve pending M0 literals or permit product work before M0 exit. Existing owners retain their domain behavior; new leaves own only shared implementation machinery or the new operation. Update effective contracts and scenario assignments with this table rather than inventing downstream evidence prerequisites.

| Revision | Contract / interface owner | Execution and evidence owner |
|---|---|---|
| Action-specific confirmation preconditions | `boring-cdc-d-security`, `boring-cdc-m1-cli-contract` | `boring-cdc-m2-schema` persists control revisions; `boring-cdc-m2-ownership` owns atomic action dependency/predicate validation through the writer interface; domains advance their revisions with mutations; `boring-cdc-m2-fault-status` only projects facts and proves integrated busy-runtime confirmation |
| Intentional safe-stop closure | `boring-cdc-d-failure-policy`, `boring-cdc-d-pg-protocol`, `boring-cdc-m0-pg-contract` | `boring-cdc-m2.1` supplies policy; `boring-cdc-m2-ownership` supplies lifecycle fencing interface; `boring-cdc-m2-capture-runtime` owns actual persist-before-close, delayed EOF and advisory-loss integration |
| Actual-writer SQLite attestation | `boring-cdc-d-sqlite`, `boring-cdc-m0-storage-model` | `boring-cdc-m2-schema` owns connection admission/readback; `boring-cdc-m1-preflight` owns read-only/offline diagnostics with synthetic live-attestation inputs; `boring-cdc-m2-fault-status` integrates actual writer evidence |
| Bounded readers and checkpoints | `boring-cdc-d-sqlite`, `boring-cdc-d-admission`, `boring-cdc-m0-storage-model` | `boring-cdc-m2-schema` owns reader handles/deadlines and connection settings; `boring-cdc-m2-journal` owns bounded range reads; `boring-cdc-m2-pressure` owns logical pin/GC and physical checkpoint scheduling; later materializers consume those APIs |
| Fair writer and control scheduling | `boring-cdc-d-admission`, `boring-cdc-m0-storage-model` | `boring-cdc-m2-journal` owns queue/service scheduling with typed consumer doubles; `boring-cdc-m2-capture-runtime` proves live feedback/probe service; `boring-cdc-m2-fault-status` integrates pressure/checkpoint/failure/command progress; M6 measures endurance |
| Typed boundaries and deterministic schedules | `boring-cdc-m1.1` owns value representations and reusable bounded harness | Existing domain leaves own production transition functions and their harness scenarios, including M1 bootstrap, M2 capture/ownership, M3 invalidation, M4/M5 promotion; `boring-cdc-m6-failure-matrix` composes them, never a second model |
| Finite refreshable audit rounds | `boring-cdc-d-ch-accept`, `boring-cdc-d-archive-durability`, respective M0 model artifacts | `boring-cdc-m2-schema` owns persistence fields; `boring-cdc-m4-durability` owns ClickHouse rounds; `boring-cdc-m5-segment-set` owns archive rounds; `boring-cdc-m5-loops` schedules the domain APIs without reimplementation |
| Measured recoverability envelope | `boring-cdc-d-scale` owns profile/equation/fixture specification, not measurements | `boring-cdc-m6-metrics` implements forecast/projection using synthetic profile evidence and existing gauges; `boring-cdc-m6-endurance` owns actual rate/outage validation; `boring-cdc-m7-docs` publishes measured limits; no metrics-to-endurance back-edge |
| Evidence-qualified event explanation | `boring-cdc-m1-cli-contract` owns schema/operation registration with synthetic examples | `boring-cdc-m5.1` solely implements the explain variant after existing domain/GC/CLI outputs; `boring-cdc-m2-reconcile` retains base journal inspection and does not require this future handler at M2 closure |

`boring-cdc-m1.1` is an M1 leaf after `boring-cdc-m0-gate`; M1 completion includes it before the terminal raw demo. `boring-cdc-m5.1` is an M5 leaf after the existing operator/GC path; M5 completion includes it before the terminal fault suite. M0 still approves specifications and named later executors only. Numeric limits, schema versions and exact profile values remain owned by the pending decisions. New interface consumers prove their own integrations when executable, not as upstream leaf prerequisites.

### M0 — Contract and irreversible decisions

Repository-tool implementation is explicitly permitted before decision approval, but only for the following bounded M0 owners. No stage approves product literals or starts M1/product-runtime work:

1. `boring-cdc-m0.1`: bootstrap decision/artifact/evidence/log/runbook schemas and validators, pinned graph primitives, synthetic fixtures; no dependency on Rust scaffold, actual approvals, or later results.
2. `boring-cdc-m0.2`: stable-ID registry/assignment coverage, world-state/effective-contract/context/impact validation, and the first read-only doctor/next/context/impact commands; consumes stage 1 and works while all decisions remain open.
3. `boring-cdc-m0.3`: immutable claims, compatibility, findings, and handoff validators; consumes stage 1, not decision approvals. Stage 2 treats absent claim indexes as explicitly pending evidence, never a pass, so neither leaf waits for the other.
4. `boring-cdc-m0-validation-tooling`: aggregate compatibility/hostile-fixture proof over all three stages, not another implementation owner. Individual decisions and their aggregate depend on stages 1 and 2, not stage 3 or this aggregate. Scaffold and M0 completion still require the full aggregate.

All original tooling obligations remain mandatory for M0 exit. Decision authors edit their own manifest rows serially in one working tree; graph readiness is not permission for concurrent writers to the shared manifest. Stage estimates are planning inputs, not a promise to complete M0 in the kickoff day.
...
- one `TxnBuffer`/`MemoryBudget` capture path, logical-decoding spill/commit-burst telemetry, and exclusive-lock orphan-spool recovery. `boring-cdc-m2.1` owns the shared pure `FailurePolicy`, scheduler/fingerprint core, and persistence adapter after schema and before spool/reconciliation/JSONL consumers. `boring-cdc-m2-capture-runtime` owns alive-but-no-reconnect `capture_safe_stopped`, real config-fingerprint re-arm, and restart/feedback integration. Spool uses explicit typed runtime doubles, not downstream runtime evidence; real ClickHouse/archive adapters run the unchanged shared vectors under their M4/M5 owners, never as M2 prerequisites;
...
- approved robustness fixtures prove busy-runtime action-scoped confirmation, expected safe-stop closure without hiding ownership loss, actual-writer SQLite attestation, reader release/WAL recycling, fair service of control/checkpoint/GC work, same-code deterministic transition schedules, and finite refreshable audit rounds; the measured recoverability envelope and bounded evidence-qualified event-explanation operation pass their named milestone owners; no upstream leaf requires later adapter or endurance results;
- no expected committed event is missing after recoverable failures; business-event delivery is proved by independent event-to-ledger correlation, separately from ledger delivery and final state, with business-only/ledger-only omission negatives and explicit unavailable managed visibility;
...
- incremental destination audits stay within per-pass byte/event/time budgets, persist cursor/freshness coverage, recompute ClickHouse hashes from actual stored payloads rather than trusting stored hash columns, detect exercised post-checkpoint ClickHouse/archive corruption and correctness-critical contract drift, publish ClickHouse `journal_verified_range` plus archive `journal_verified_range`/`self_consistent_range`, and never infer gaps/older integrity;
...
This register is an implementation gate, not a future-ideas list. A row marked `Open`, `Confirm`, or `Owner approval` is blocking when it changes capture durability, source ordering, source safety, destination correctness, replay, public security, or a published capacity claim. **For M0 closure, “validation evidence” means an approved, mechanically usable fixture specification plus the named later Bead that will execute it. Runtime pass/fail results are never prerequisites for M0.** M0 **fails** while a contract, owner, fixture specification, or executing-Bead assignment remains unresolved; it does not wait for M1–M7 implementations. Before those rows close, only planning/decision work, the named repository-only M0 tooling stages and their aggregate proof in section 16, and non-product series/access preparation may proceed. This is an explicit bootstrap exception, not permission for M1/product-runtime, journal, or destination implementation or approval bypass.
...
| Advisory-lock or unexpected CopyBoth session loss while other connections live | Old and successor runtimes can both dispatch, or an in-place reopen weakens fencing | Unexpected loss is ownership loss in v0.1; a matching persist-before-close deterministic safe-stop follows section 3 without suppressing advisory-lock loss; dedicated non-pooled lock session, deadline-fit admission for source-mutating dispatch, generation-fenced immutable destination intents/namespaces, fail/exit without in-place reacquire, persisted process-owned reconnect schedule, unlimited supervised restarts, bounded zombie-session reap, successor wait plus full reconciliation |
...
| Ledger and final-state checks hide a dropped intermediate business event | False correctness result when the separate ledger arrives and a later update overwrites the loss | Independently correlate actual business events with the expected ledger; business-only and ledger-only omission negatives, transaction multiplicity/delete cases, and separate baseline/ledger/event/final-state claims |
